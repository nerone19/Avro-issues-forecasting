...
#frequenc num transition for last transition
...
#let's check for which priority an issue is usually resolved
d = df[df['status'] == 'Resolved']
ax = sns.countplot(x="priority",  data=d).set_title('priority of solved issues')
plt.show()

#let's create a new feature which report how many minutes were spent before solving the issue
for index, row in d.iterrows():
    parsed_t = dp.parse(row['resolutiondate'])
    resoution_in_minutes =(parsed_t.timestamp())/60
    
    parsed_t = dp.parse(row['created'])
    creation_in_minutes =(parsed_t.timestamp())/60
    time_spent_in_minutes = resoution_in_minutes - creation_in_minutes
    d.loc[index,'time_for_solving'] = time_spent_in_minutes


sns.scatterplot(data=d, x="total_bill", y="time_for_solving")
sns.distplot(d['time_for_solving']);
